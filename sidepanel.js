...
// 防止传递链把不同产品逐步并入同一组。
// V21.0.70: 指纹优先 — 若双方指纹均存在且不同，直接拒合
...
var lengthRatio = Math.min(t1.length, t2.length) / Math.max(t1.length, t2.length);
  ...
  // V21.0.70: 边际区 0.88-0.94 不自动合，入待确认（宁可漏合不误合）
      ...
      if (ts === 0 && typeof dateVal === 'string') {
        ...
        detectTableFields(PAT, PTT).then(function(procFieldMap) {
          ...
          getToken().then(function(t) {
            ...
            // V21.0.70: 收集 PENDING_REVIEW 入待确认
...
var SCHEDULE_CONFIG = {
  accounts: [
    ...
    {tail: '7205', name: '豆子', platforms: ['拼多多','京东','淘宝','抖音一区','抖音二区','快手小店','快手小店二区','微信小店','微信小店二区'], urlToken: 'E9AD6D4CCE7DB911DC8FDE8A54EFF2C0'},
    {tail: '7207', name: 'A售后', platforms: ['拼多多','京东','淘宝','抖音一区','抖音二区','快手小店','快手小店二区','微信小店','微信小店二区'], urlToken: '382346A2BAB84AEA0060104B84DCD1DF'}
  ],
...
function runScheduleQueue(options) {
  ...
  return loadScheduleQueue().then(function(state) {
    ...
    if (options.forceNew || !queue.length || queue[0].businessDate !== todayKey || queue.length !== 19) {
    ...
    L('[调度] 7205(豆子): 9区 (含二区)', 'i');
    L('[调度] 7207(A售后): 9区 (含二区)', 'i');
...
var TK_ACCOUNTS = [
  ...
  {tail:'7205', name:'豆子', platforms:['拼多多','京东','淘宝','抖音一区','抖音二区','快手小店','快手小店二区','微信小店','微信小店二区'], css:'g7205'},
  {tail:'7207', name:'A售后', platforms:['拼多多','京东','淘宝','抖音一区','抖音二区','快手小店','快手小店二区','微信小店','微信小店二区'], css:'g7207'}
];
var _tkData = {};
